#!/bin/bash
# Memory profiling script for SFT on DGX Spark
#
# Usage: ./scripts/memory_profile_sft.sh <batch_size> <grad_accum> <seq_len> [model]
#
# Example:
#   ./scripts/memory_profile_sft.sh 1 1 1024
#   ./scripts/memory_profile_sft.sh 8 4 1024 Qwen/Qwen2.5-1.5B
#
# This runs a SHORT training job (50 steps) to measure peak memory usage.
# Run in FOREGROUND and monitor memory in another terminal:
...
echo "========================================"
echo "DGX Spark Memory Profiling - SFT"
echo "========================================"
echo "Model:           $MODEL"
...
OUTPUT_DIR="/tmp/memory_profile_${MODEL_SHORT}_b${BATCH_SIZE}_g${GRAD_ACCUM}_s${SEQ_LEN}"
rm -rf "$OUTPUT_DIR"

# Memory monitoring file
MEM_LOG="/tmp/mem_monitor_$$.log"
PEAK_MEM_FILE="/tmp/peak_mem_$$.txt"
...
# NOTE: Adjust the path to your training script
uv run python -m accelerate.commands.launch \
    --mixed_precision bf16 \
    --num_processes 1 \
    open_instruct/finetune.py \
    --exp_name "mem_profile_${MODEL_SHORT}" \
...
echo ""
echo "========================================"
echo "RESULT: batch=$BATCH_SIZE grad_accum=$GRAD_ACCUM seq=$SEQ_LEN peak=${PEAK_MEM}GB model=$MODEL"
echo "========================================"
